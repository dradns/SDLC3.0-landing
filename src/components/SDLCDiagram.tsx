        ...
        <div className="relative">
          ...
          <div className="relative h-[400px] sm:h-[500px] bg-gradient-card rounded-2xl border border-card-border shadow-custom-lg overflow-hidden">
            ...
            <svg className="absolute inset-0 w-full h-full pointer-events-none">
              ...
              return <line key={index} x1={`${currentPos.x}%`} y1={`${currentPos.y}%`} x2={`${nextPos.x}%`} y2={`${nextPos.y}%`} stroke="hsl(var(--primary))" strokeWidth="2" strokeDasharray="8,4" className={`transition-all duration-300 ${activeStep === index || activeStep === (index + 1) % steps.length ? 'opacity-100' : 'opacity-20'}`} />;
            })}
              
              {/* Lines from center to each step */}
              {steps.map((step, index) => {
              const stepPos = getStepPosition(index, steps.length);
              return <line key={`center-${index}`} x1="50%" y1="50%" x2={`${stepPos.x}%`} y2={`${stepPos.y}%`} stroke="hsl(var(--primary))" strokeWidth="1" strokeDasharray="3,3" className={`transition-all duration-300 ${activeStep === index ? 'opacity-60' : 'opacity-10'}`} />;
            })}
            </svg>
            ...
            return <div key={step.id} className={`absolute transform -translate-x-1/2 -translate-y-1/2 cursor-pointer transition-all duration-300 ${activeStep === step.id ? 'scale-110 z-10' : 'scale-100 hover:scale-105'}`} style={{
              left: `${position.x}%`,
              top: `${position.y}%`
                  ...
                  {/* Step label */}
                  <div className="absolute left-1/2 transform -translate-x-1/2 text-center" style={{
